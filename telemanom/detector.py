...
class Detector:
    def __init__(self, labels_path=None, result_path='results/',
                 config_path='config.yaml', Path=None):
        """
        ...
        if labels_path is not None:
            self.labels_path = os.path.join(Path, labels_path)
        ...
        self.config = Config(os.path.join(Path, config_path))
        self.y_hat = None
        ...
        helpers.make_dirs(self.id, Config=self.config, Path=Path)
        ...
        self.result_path = os.path.join(Path, result_path)
        ...
        if self.labels_path:
            ...
            chan_ids = [x.split('.')[0] for x in os.listdir(data_dir)]
    ...
    def run(self):
        ...
        for i, row in self.chan_df.iterrows():
            ...
            channel.load_data(self.Path)

            if self.config.predict:
                #model = Model(self.config, self.id, channel, self.Path, False)
                # initialize Model without training
                model = Model(self.config, self.id, self.Path)
                if not model.load(self.Path):
                    model.train_new(channel)
                    model.save(self.Path)
                model_list.append(model)
                channel = model.batch_predict(channel, Train=False, Path=self.Path)
            else:
                channel.y_hat = np.load(os.path.join(self.Path, 'data', self.id, 'y_hat',
                                                     '{}.npy'
                                                     .format(channel.id)))

            errors = Errors(channel, self.config, self.id, self.Path)
