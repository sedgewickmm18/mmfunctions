...
class Plotter:
    def __init__(self, run_id, config_path='config.yaml', Path=None):
        """
    ...
    def plot_channel(self, channel_id, plot_train=False, plot_errors=True):
        ...
        plot_values = {
            'y_hat': np.load(os.path.join(self.Path, 'data', self.run_id, 'y_hat',
                                          '{}.npy'.format(channel_id))),
            'smoothed_errors': np.load(os.path.join(self.Path, 'data', self.run_id,
                                                    'smoothed_errors',
                                                    '{}.npy'.format(channel_id))),
            'test': np.load(os.path.join(self.Path, 'data', 'test', '{}.npy'
                                         .format(channel_id))),
            'train': np.load(os.path.join(self.Path, 'data', 'train', '{}.npy'
                                          .format(channel_id)))
        }
        ...
        return train_df, y_df, e_df

        if plot_train:
            train_df.plot(kind='scatter')
            #train_df.plot(kind='scatter', color='green',
            #               layout={'title': "Training Data"})

        y_df.plot(kind='scatter') #, layout=y_layout)

        if plot_errors:
            e_df.plot(kind='scatter') #, layout=e_layout, color='red')
