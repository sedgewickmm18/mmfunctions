...
class InvokeWMLModelBase(BaseTransformer):
    ...
    def __init__(self, input_items, wml_auth, output_items, time_column=None):
        super().__init__()
    ...
    def _calc(self, df):
        ...
        if results:
            ...
            if result_json is not None:
                ...
                new_df = pd.DataFrame(
                    data=preddata, index=timedata, columns=self.output_items
                )

                df = df.drop(columns=self.output_items.extend(self.time_column))
                pd.merge_asof(
                    df, new_df, left_index=True, right_index=True, direction="nearest"
                )
...
class InvokeWMLModelWithTime(InvokeWMLModelBase):
    ...
    def __init__(self, input_items, wml_auth, output_items, time_column):
        super().__init__(input_items, wml_auth, output_items, time_column)
